# -*- coding: utf-8 -*-
"""Generate submission-ready v19 from v15.

Changes (on top of v18):
...
ABSTRACT = [
    ...
    ("Materials and Methods. ",
     ...
     "(n = 1,979), with classifier validation in GSE21653 and GSE7390 and "
     "nearest-centroid subtype mapping across all external cohorts."),
    ("Results. ",
...
BODY = {
    ...
    "2.1 Data sources and verification":
        ("2.1 Data sources and verification ",
         ...
         "atlas) [12] was used for single-cell validation. TCGA-BRCA "
    ...
    "3.3 Hub genes and the ER-status classifier":
        ("3.3 Hub genes and the ER-status classifier ",
         ...
         "100%, PRKAA1 99.7%, LIMA1 73.7%), supporting selection stability. "
    ...
    "3.5 Immune microenvironment and pathway programs":
        ("3.5 Immune microenvironment and pathway programs across subtypes\n",
         ...
         "Kruskal-Wallis P = 1.9e-3; StromalScore highest in C2, "
         "P = 4.2e-22; ESTIMATEScore P = 3.5e-6), and CIBERSORT showed "
         "significantly higher C1 fractions of M1 and M0 macrophages, "
         "activated dendritic cells, follicular helper T cells, monocytes "
         "and activated NK cells, with higher C2/C3 fractions of resting "
         "mast cells and M2 macrophages (all Kruskal-Wallis P < 0.001). "
         "Immune-checkpoint genes CD274, PDCD1, CTLA4, LAG3 and IDO1 were "
         "highest in C1 (Kruskal-Wallis P = 1.7e-20 to 4.9e-5), while HAVCR2 "
         "and CD8A did not differ across subtypes (P = 0.12 and P = 0.11, "
         "respectively; Supplementary Tables S8 and S14)."),
    ...
    "3.9 Colocalisation supports":
        ("3.9 Colocalisation supports a shared causal variant at the FDPS locus\n",
         ...
         "extended to the three additional eQTLGen-supported genes (LIPA, "
         "FAXDC2, SREBF1); none showed strong evidence of a shared causal "
         "variant with breast cancer risk (PP.H4 = 0.035, 0.043 and 0.171, "
         "respectively; PP.H1 dominated), indicating that their SMR signals "
         "are driven by strong eQTL associations without a corresponding "
    ...
    "3.10 FDPS expression and clinical features":
        ("3.10 FDPS expression and clinical features\n",
         ...
         "(P = 0.195; HPA validation cohort P = 0.023, classified as "
         "unprognostic), consistent with the transcript-level null. FDPS mRNA "
         "was detected in all 17 cancer types with available cBioPortal "
         "RNA-sequencing data, indicating pan-cancer expression "
    ...
    "4.1 Principal findings":
        ("4.1 Principal findings ",
         ...
         "METABRIC 0.922; GSE21653 0.847; GSE7390 0.903). Consensus clustering "
    ...
    "4.6 Causal inference completed":
        ("4.6 Causal inference ",
         ...
         "expression on breast cancer risk [43-45]. The protective direction "
         ...
         "reduce ER+ breast cancer risk [46] and with the established adjuvant "
         "benefit of nitrogen-containing bisphosphonates, which inhibit FDPS "
         "[47]. Multi-signal colocalisation was less conclusive (maximum "
         "PP.H4 = 0.44)."),
...
NEW_SECTIONS = [
    ("4.7 Comparison with published lipid-metabolism signatures\n",
     ...
     "were identified by systematic PubMed/Europe PMC search and compared on "
     "cohort use, signature size, immune deconvolution, subtype analysis, "
     "single-cell evidence and causal inference [7,48-52] (Supplementary "
     "Table S11). All six derived prognostic risk scores from TCGA-BRCA with "
     "LASSO-type feature selection; two validated across multiple "
     "independent cohorts, none performed germline causal inference "
     "(SMR/MR or Bayesian colocalisation), and none combined CIBERSORT/"
     "ESTIMATE deconvolution with external subtype mapping. To our "
     "knowledge, this is the first cholesterol-metabolism-focused breast "
     "cancer study to combine cross-platform ER-classifier validation, "
     "externally mapped molecular subtypes, immune deconvolution, "
     "single-cell localization and germline causal inference in one "
     "evidence chain."),
]
